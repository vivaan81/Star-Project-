// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from tuos_interfaces:action/CameraSweep.idl
// generated code does not contain a copyright notice
#include "tuos_interfaces/action/detail/camera_sweep__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
tuos_interfaces__action__CameraSweep_Goal__init(tuos_interfaces__action__CameraSweep_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // sweep_angle
  // image_count
  return true;
}

void
tuos_interfaces__action__CameraSweep_Goal__fini(tuos_interfaces__action__CameraSweep_Goal * msg)
{
  if (!msg) {
    return;
  }
  // sweep_angle
  // image_count
}

bool
tuos_interfaces__action__CameraSweep_Goal__are_equal(const tuos_interfaces__action__CameraSweep_Goal * lhs, const tuos_interfaces__action__CameraSweep_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // sweep_angle
  if (lhs->sweep_angle != rhs->sweep_angle) {
    return false;
  }
  // image_count
  if (lhs->image_count != rhs->image_count) {
    return false;
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_Goal__copy(
  const tuos_interfaces__action__CameraSweep_Goal * input,
  tuos_interfaces__action__CameraSweep_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // sweep_angle
  output->sweep_angle = input->sweep_angle;
  // image_count
  output->image_count = input->image_count;
  return true;
}

tuos_interfaces__action__CameraSweep_Goal *
tuos_interfaces__action__CameraSweep_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Goal * msg = (tuos_interfaces__action__CameraSweep_Goal *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tuos_interfaces__action__CameraSweep_Goal));
  bool success = tuos_interfaces__action__CameraSweep_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tuos_interfaces__action__CameraSweep_Goal__destroy(tuos_interfaces__action__CameraSweep_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tuos_interfaces__action__CameraSweep_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tuos_interfaces__action__CameraSweep_Goal__Sequence__init(tuos_interfaces__action__CameraSweep_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Goal * data = NULL;

  if (size) {
    data = (tuos_interfaces__action__CameraSweep_Goal *)allocator.zero_allocate(size, sizeof(tuos_interfaces__action__CameraSweep_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tuos_interfaces__action__CameraSweep_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tuos_interfaces__action__CameraSweep_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tuos_interfaces__action__CameraSweep_Goal__Sequence__fini(tuos_interfaces__action__CameraSweep_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tuos_interfaces__action__CameraSweep_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tuos_interfaces__action__CameraSweep_Goal__Sequence *
tuos_interfaces__action__CameraSweep_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Goal__Sequence * array = (tuos_interfaces__action__CameraSweep_Goal__Sequence *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tuos_interfaces__action__CameraSweep_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tuos_interfaces__action__CameraSweep_Goal__Sequence__destroy(tuos_interfaces__action__CameraSweep_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tuos_interfaces__action__CameraSweep_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tuos_interfaces__action__CameraSweep_Goal__Sequence__are_equal(const tuos_interfaces__action__CameraSweep_Goal__Sequence * lhs, const tuos_interfaces__action__CameraSweep_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_Goal__Sequence__copy(
  const tuos_interfaces__action__CameraSweep_Goal__Sequence * input,
  tuos_interfaces__action__CameraSweep_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tuos_interfaces__action__CameraSweep_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tuos_interfaces__action__CameraSweep_Goal * data =
      (tuos_interfaces__action__CameraSweep_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tuos_interfaces__action__CameraSweep_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tuos_interfaces__action__CameraSweep_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `image_path`
#include "rosidl_runtime_c/string_functions.h"

bool
tuos_interfaces__action__CameraSweep_Result__init(tuos_interfaces__action__CameraSweep_Result * msg)
{
  if (!msg) {
    return false;
  }
  // image_path
  if (!rosidl_runtime_c__String__init(&msg->image_path)) {
    tuos_interfaces__action__CameraSweep_Result__fini(msg);
    return false;
  }
  return true;
}

void
tuos_interfaces__action__CameraSweep_Result__fini(tuos_interfaces__action__CameraSweep_Result * msg)
{
  if (!msg) {
    return;
  }
  // image_path
  rosidl_runtime_c__String__fini(&msg->image_path);
}

bool
tuos_interfaces__action__CameraSweep_Result__are_equal(const tuos_interfaces__action__CameraSweep_Result * lhs, const tuos_interfaces__action__CameraSweep_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // image_path
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->image_path), &(rhs->image_path)))
  {
    return false;
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_Result__copy(
  const tuos_interfaces__action__CameraSweep_Result * input,
  tuos_interfaces__action__CameraSweep_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // image_path
  if (!rosidl_runtime_c__String__copy(
      &(input->image_path), &(output->image_path)))
  {
    return false;
  }
  return true;
}

tuos_interfaces__action__CameraSweep_Result *
tuos_interfaces__action__CameraSweep_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Result * msg = (tuos_interfaces__action__CameraSweep_Result *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tuos_interfaces__action__CameraSweep_Result));
  bool success = tuos_interfaces__action__CameraSweep_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tuos_interfaces__action__CameraSweep_Result__destroy(tuos_interfaces__action__CameraSweep_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tuos_interfaces__action__CameraSweep_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tuos_interfaces__action__CameraSweep_Result__Sequence__init(tuos_interfaces__action__CameraSweep_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Result * data = NULL;

  if (size) {
    data = (tuos_interfaces__action__CameraSweep_Result *)allocator.zero_allocate(size, sizeof(tuos_interfaces__action__CameraSweep_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tuos_interfaces__action__CameraSweep_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tuos_interfaces__action__CameraSweep_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tuos_interfaces__action__CameraSweep_Result__Sequence__fini(tuos_interfaces__action__CameraSweep_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tuos_interfaces__action__CameraSweep_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tuos_interfaces__action__CameraSweep_Result__Sequence *
tuos_interfaces__action__CameraSweep_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Result__Sequence * array = (tuos_interfaces__action__CameraSweep_Result__Sequence *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tuos_interfaces__action__CameraSweep_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tuos_interfaces__action__CameraSweep_Result__Sequence__destroy(tuos_interfaces__action__CameraSweep_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tuos_interfaces__action__CameraSweep_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tuos_interfaces__action__CameraSweep_Result__Sequence__are_equal(const tuos_interfaces__action__CameraSweep_Result__Sequence * lhs, const tuos_interfaces__action__CameraSweep_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_Result__Sequence__copy(
  const tuos_interfaces__action__CameraSweep_Result__Sequence * input,
  tuos_interfaces__action__CameraSweep_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tuos_interfaces__action__CameraSweep_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tuos_interfaces__action__CameraSweep_Result * data =
      (tuos_interfaces__action__CameraSweep_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tuos_interfaces__action__CameraSweep_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tuos_interfaces__action__CameraSweep_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
tuos_interfaces__action__CameraSweep_Feedback__init(tuos_interfaces__action__CameraSweep_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_image
  // current_angle
  return true;
}

void
tuos_interfaces__action__CameraSweep_Feedback__fini(tuos_interfaces__action__CameraSweep_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_image
  // current_angle
}

bool
tuos_interfaces__action__CameraSweep_Feedback__are_equal(const tuos_interfaces__action__CameraSweep_Feedback * lhs, const tuos_interfaces__action__CameraSweep_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_image
  if (lhs->current_image != rhs->current_image) {
    return false;
  }
  // current_angle
  if (lhs->current_angle != rhs->current_angle) {
    return false;
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_Feedback__copy(
  const tuos_interfaces__action__CameraSweep_Feedback * input,
  tuos_interfaces__action__CameraSweep_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_image
  output->current_image = input->current_image;
  // current_angle
  output->current_angle = input->current_angle;
  return true;
}

tuos_interfaces__action__CameraSweep_Feedback *
tuos_interfaces__action__CameraSweep_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Feedback * msg = (tuos_interfaces__action__CameraSweep_Feedback *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tuos_interfaces__action__CameraSweep_Feedback));
  bool success = tuos_interfaces__action__CameraSweep_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tuos_interfaces__action__CameraSweep_Feedback__destroy(tuos_interfaces__action__CameraSweep_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tuos_interfaces__action__CameraSweep_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tuos_interfaces__action__CameraSweep_Feedback__Sequence__init(tuos_interfaces__action__CameraSweep_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Feedback * data = NULL;

  if (size) {
    data = (tuos_interfaces__action__CameraSweep_Feedback *)allocator.zero_allocate(size, sizeof(tuos_interfaces__action__CameraSweep_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tuos_interfaces__action__CameraSweep_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tuos_interfaces__action__CameraSweep_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tuos_interfaces__action__CameraSweep_Feedback__Sequence__fini(tuos_interfaces__action__CameraSweep_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tuos_interfaces__action__CameraSweep_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tuos_interfaces__action__CameraSweep_Feedback__Sequence *
tuos_interfaces__action__CameraSweep_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_Feedback__Sequence * array = (tuos_interfaces__action__CameraSweep_Feedback__Sequence *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tuos_interfaces__action__CameraSweep_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tuos_interfaces__action__CameraSweep_Feedback__Sequence__destroy(tuos_interfaces__action__CameraSweep_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tuos_interfaces__action__CameraSweep_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tuos_interfaces__action__CameraSweep_Feedback__Sequence__are_equal(const tuos_interfaces__action__CameraSweep_Feedback__Sequence * lhs, const tuos_interfaces__action__CameraSweep_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_Feedback__Sequence__copy(
  const tuos_interfaces__action__CameraSweep_Feedback__Sequence * input,
  tuos_interfaces__action__CameraSweep_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tuos_interfaces__action__CameraSweep_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tuos_interfaces__action__CameraSweep_Feedback * data =
      (tuos_interfaces__action__CameraSweep_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tuos_interfaces__action__CameraSweep_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tuos_interfaces__action__CameraSweep_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "tuos_interfaces/action/detail/camera_sweep__functions.h"

bool
tuos_interfaces__action__CameraSweep_SendGoal_Request__init(tuos_interfaces__action__CameraSweep_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    tuos_interfaces__action__CameraSweep_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!tuos_interfaces__action__CameraSweep_Goal__init(&msg->goal)) {
    tuos_interfaces__action__CameraSweep_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
tuos_interfaces__action__CameraSweep_SendGoal_Request__fini(tuos_interfaces__action__CameraSweep_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  tuos_interfaces__action__CameraSweep_Goal__fini(&msg->goal);
}

bool
tuos_interfaces__action__CameraSweep_SendGoal_Request__are_equal(const tuos_interfaces__action__CameraSweep_SendGoal_Request * lhs, const tuos_interfaces__action__CameraSweep_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!tuos_interfaces__action__CameraSweep_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_SendGoal_Request__copy(
  const tuos_interfaces__action__CameraSweep_SendGoal_Request * input,
  tuos_interfaces__action__CameraSweep_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!tuos_interfaces__action__CameraSweep_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

tuos_interfaces__action__CameraSweep_SendGoal_Request *
tuos_interfaces__action__CameraSweep_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_SendGoal_Request * msg = (tuos_interfaces__action__CameraSweep_SendGoal_Request *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Request));
  bool success = tuos_interfaces__action__CameraSweep_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tuos_interfaces__action__CameraSweep_SendGoal_Request__destroy(tuos_interfaces__action__CameraSweep_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tuos_interfaces__action__CameraSweep_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence__init(tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_SendGoal_Request * data = NULL;

  if (size) {
    data = (tuos_interfaces__action__CameraSweep_SendGoal_Request *)allocator.zero_allocate(size, sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tuos_interfaces__action__CameraSweep_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tuos_interfaces__action__CameraSweep_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence__fini(tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tuos_interfaces__action__CameraSweep_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence *
tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence * array = (tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence__destroy(tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence__are_equal(const tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence * lhs, const tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence__copy(
  const tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence * input,
  tuos_interfaces__action__CameraSweep_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tuos_interfaces__action__CameraSweep_SendGoal_Request * data =
      (tuos_interfaces__action__CameraSweep_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tuos_interfaces__action__CameraSweep_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tuos_interfaces__action__CameraSweep_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
tuos_interfaces__action__CameraSweep_SendGoal_Response__init(tuos_interfaces__action__CameraSweep_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    tuos_interfaces__action__CameraSweep_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
tuos_interfaces__action__CameraSweep_SendGoal_Response__fini(tuos_interfaces__action__CameraSweep_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
tuos_interfaces__action__CameraSweep_SendGoal_Response__are_equal(const tuos_interfaces__action__CameraSweep_SendGoal_Response * lhs, const tuos_interfaces__action__CameraSweep_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_SendGoal_Response__copy(
  const tuos_interfaces__action__CameraSweep_SendGoal_Response * input,
  tuos_interfaces__action__CameraSweep_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

tuos_interfaces__action__CameraSweep_SendGoal_Response *
tuos_interfaces__action__CameraSweep_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_SendGoal_Response * msg = (tuos_interfaces__action__CameraSweep_SendGoal_Response *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Response));
  bool success = tuos_interfaces__action__CameraSweep_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tuos_interfaces__action__CameraSweep_SendGoal_Response__destroy(tuos_interfaces__action__CameraSweep_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tuos_interfaces__action__CameraSweep_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence__init(tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_SendGoal_Response * data = NULL;

  if (size) {
    data = (tuos_interfaces__action__CameraSweep_SendGoal_Response *)allocator.zero_allocate(size, sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tuos_interfaces__action__CameraSweep_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tuos_interfaces__action__CameraSweep_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence__fini(tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tuos_interfaces__action__CameraSweep_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence *
tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence * array = (tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence__destroy(tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence__are_equal(const tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence * lhs, const tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence__copy(
  const tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence * input,
  tuos_interfaces__action__CameraSweep_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tuos_interfaces__action__CameraSweep_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tuos_interfaces__action__CameraSweep_SendGoal_Response * data =
      (tuos_interfaces__action__CameraSweep_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tuos_interfaces__action__CameraSweep_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tuos_interfaces__action__CameraSweep_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
tuos_interfaces__action__CameraSweep_GetResult_Request__init(tuos_interfaces__action__CameraSweep_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    tuos_interfaces__action__CameraSweep_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
tuos_interfaces__action__CameraSweep_GetResult_Request__fini(tuos_interfaces__action__CameraSweep_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
tuos_interfaces__action__CameraSweep_GetResult_Request__are_equal(const tuos_interfaces__action__CameraSweep_GetResult_Request * lhs, const tuos_interfaces__action__CameraSweep_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_GetResult_Request__copy(
  const tuos_interfaces__action__CameraSweep_GetResult_Request * input,
  tuos_interfaces__action__CameraSweep_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

tuos_interfaces__action__CameraSweep_GetResult_Request *
tuos_interfaces__action__CameraSweep_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_GetResult_Request * msg = (tuos_interfaces__action__CameraSweep_GetResult_Request *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tuos_interfaces__action__CameraSweep_GetResult_Request));
  bool success = tuos_interfaces__action__CameraSweep_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tuos_interfaces__action__CameraSweep_GetResult_Request__destroy(tuos_interfaces__action__CameraSweep_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tuos_interfaces__action__CameraSweep_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence__init(tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_GetResult_Request * data = NULL;

  if (size) {
    data = (tuos_interfaces__action__CameraSweep_GetResult_Request *)allocator.zero_allocate(size, sizeof(tuos_interfaces__action__CameraSweep_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tuos_interfaces__action__CameraSweep_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tuos_interfaces__action__CameraSweep_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence__fini(tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tuos_interfaces__action__CameraSweep_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence *
tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence * array = (tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence__destroy(tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence__are_equal(const tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence * lhs, const tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence__copy(
  const tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence * input,
  tuos_interfaces__action__CameraSweep_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tuos_interfaces__action__CameraSweep_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tuos_interfaces__action__CameraSweep_GetResult_Request * data =
      (tuos_interfaces__action__CameraSweep_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tuos_interfaces__action__CameraSweep_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tuos_interfaces__action__CameraSweep_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "tuos_interfaces/action/detail/camera_sweep__functions.h"

bool
tuos_interfaces__action__CameraSweep_GetResult_Response__init(tuos_interfaces__action__CameraSweep_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!tuos_interfaces__action__CameraSweep_Result__init(&msg->result)) {
    tuos_interfaces__action__CameraSweep_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
tuos_interfaces__action__CameraSweep_GetResult_Response__fini(tuos_interfaces__action__CameraSweep_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  tuos_interfaces__action__CameraSweep_Result__fini(&msg->result);
}

bool
tuos_interfaces__action__CameraSweep_GetResult_Response__are_equal(const tuos_interfaces__action__CameraSweep_GetResult_Response * lhs, const tuos_interfaces__action__CameraSweep_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!tuos_interfaces__action__CameraSweep_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_GetResult_Response__copy(
  const tuos_interfaces__action__CameraSweep_GetResult_Response * input,
  tuos_interfaces__action__CameraSweep_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!tuos_interfaces__action__CameraSweep_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

tuos_interfaces__action__CameraSweep_GetResult_Response *
tuos_interfaces__action__CameraSweep_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_GetResult_Response * msg = (tuos_interfaces__action__CameraSweep_GetResult_Response *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tuos_interfaces__action__CameraSweep_GetResult_Response));
  bool success = tuos_interfaces__action__CameraSweep_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tuos_interfaces__action__CameraSweep_GetResult_Response__destroy(tuos_interfaces__action__CameraSweep_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tuos_interfaces__action__CameraSweep_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence__init(tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_GetResult_Response * data = NULL;

  if (size) {
    data = (tuos_interfaces__action__CameraSweep_GetResult_Response *)allocator.zero_allocate(size, sizeof(tuos_interfaces__action__CameraSweep_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tuos_interfaces__action__CameraSweep_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tuos_interfaces__action__CameraSweep_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence__fini(tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tuos_interfaces__action__CameraSweep_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence *
tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence * array = (tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence__destroy(tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence__are_equal(const tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence * lhs, const tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence__copy(
  const tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence * input,
  tuos_interfaces__action__CameraSweep_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tuos_interfaces__action__CameraSweep_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tuos_interfaces__action__CameraSweep_GetResult_Response * data =
      (tuos_interfaces__action__CameraSweep_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tuos_interfaces__action__CameraSweep_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tuos_interfaces__action__CameraSweep_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "tuos_interfaces/action/detail/camera_sweep__functions.h"

bool
tuos_interfaces__action__CameraSweep_FeedbackMessage__init(tuos_interfaces__action__CameraSweep_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    tuos_interfaces__action__CameraSweep_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!tuos_interfaces__action__CameraSweep_Feedback__init(&msg->feedback)) {
    tuos_interfaces__action__CameraSweep_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
tuos_interfaces__action__CameraSweep_FeedbackMessage__fini(tuos_interfaces__action__CameraSweep_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  tuos_interfaces__action__CameraSweep_Feedback__fini(&msg->feedback);
}

bool
tuos_interfaces__action__CameraSweep_FeedbackMessage__are_equal(const tuos_interfaces__action__CameraSweep_FeedbackMessage * lhs, const tuos_interfaces__action__CameraSweep_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!tuos_interfaces__action__CameraSweep_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_FeedbackMessage__copy(
  const tuos_interfaces__action__CameraSweep_FeedbackMessage * input,
  tuos_interfaces__action__CameraSweep_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!tuos_interfaces__action__CameraSweep_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

tuos_interfaces__action__CameraSweep_FeedbackMessage *
tuos_interfaces__action__CameraSweep_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_FeedbackMessage * msg = (tuos_interfaces__action__CameraSweep_FeedbackMessage *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tuos_interfaces__action__CameraSweep_FeedbackMessage));
  bool success = tuos_interfaces__action__CameraSweep_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tuos_interfaces__action__CameraSweep_FeedbackMessage__destroy(tuos_interfaces__action__CameraSweep_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tuos_interfaces__action__CameraSweep_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence__init(tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_FeedbackMessage * data = NULL;

  if (size) {
    data = (tuos_interfaces__action__CameraSweep_FeedbackMessage *)allocator.zero_allocate(size, sizeof(tuos_interfaces__action__CameraSweep_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tuos_interfaces__action__CameraSweep_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tuos_interfaces__action__CameraSweep_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence__fini(tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tuos_interfaces__action__CameraSweep_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence *
tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence * array = (tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence *)allocator.allocate(sizeof(tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence__destroy(tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence__are_equal(const tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence * lhs, const tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence__copy(
  const tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence * input,
  tuos_interfaces__action__CameraSweep_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tuos_interfaces__action__CameraSweep_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tuos_interfaces__action__CameraSweep_FeedbackMessage * data =
      (tuos_interfaces__action__CameraSweep_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tuos_interfaces__action__CameraSweep_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tuos_interfaces__action__CameraSweep_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tuos_interfaces__action__CameraSweep_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
